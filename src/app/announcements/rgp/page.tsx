...
import { Reveal } from "@/app/case-studies/reveal"
import { AnnouncementToc } from "./toc"
...
export default function RgpAnnouncementPage() {
  return (
    <main className="flex min-h-screen flex-col">
      ...
      <section className="mx-auto w-full max-w-[82.5rem] px-6 pt-10 sm:px-10 lg:pt-12">
        <Reveal>
      ...
      <section className="mx-auto flex w-full max-w-[68.75rem] flex-col items-center gap-6 px-6 pt-20 text-center sm:px-10 lg:pt-24">
        <Reveal delay={0.1}>
          <p className="text-[0.8125rem] text-black/40">July 12, 2026 · Partnership</p>
        </Reveal>
        <Reveal delay={0.18}>
        ...
        <Reveal delay={0.26}>
          <p className="max-w-[56ch] text-balance text-[1.0625rem] leading-[1.55] text-black/60">
      ...
      {/* ── Body: sticky scrollspy TOC + article ────────────────────────── */}
      <section className="mx-auto mt-20 flex w-full max-w-[68.75rem] flex-col items-start gap-12 px-6 sm:px-10 lg:mt-24 lg:flex-row lg:gap-20">
        <AnnouncementToc items={SECTIONS.map((s) => ({ id: s.id, label: s.toc }))} />

        <article className="flex min-w-0 flex-1 flex-col gap-14">
          {SECTIONS.map((s, i) => (
            <Reveal key={s.id} delay={0.05 + (i % 2) * 0.05}>
              <section id={s.id} className="flex scroll-mt-10 flex-col gap-5">
                <h2 className="text-xl font-medium leading-[1.2] tracking-[-0.02em]">
                  {s.heading}
                </h2>
                <div className="flex flex-col gap-5 text-[0.9375rem] leading-[1.6] text-black/70">
      ...
      {/* ── Pull quote ──────────────────────────────────────────────────── */}
      <section className="mx-auto flex w-full max-w-[53.75rem] flex-col items-center gap-8 px-6 pt-24 text-center sm:px-10 lg:pt-28">
        <Reveal>
        ...
        <Reveal delay={0.1} className="flex flex-col items-center gap-8">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-2 rounded-full bg-black px-6 py-3 text-sm text-white transition-colors hover:bg-black/80"
          >
            Work with Source
